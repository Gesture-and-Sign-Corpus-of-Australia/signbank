...
# Line-by-line of Ecto schema
- type
	- The 'type' of sign. The only two values at the moment are `citation` and `variant` where `variant` represents a 'variation' of the same sign with the same meaning but different handshapes, movements etc.
- id_gloss
	- A gloss used to identify a given sign, eg `house1b`
- 
