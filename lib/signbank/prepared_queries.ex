defmodule Signbank.Dictionary.PreparedQueries do
  ...
  def keyword_mismatch() do
    contains_keyword? = fn
      keyword ->
        fn definition -> definition.text
        |> String.downcase
        |> String.contains?(
          String.replace(String.downcase(keyword), ~r"\s?\(.*\)", "")
        ) end
    end
    ...
    |> Enum.filter(fn
      %{citation: nil} = s -> defs = s.definitions
        not Enum.any?(s.keywords, & Enum.any?(defs, contains_keyword?.(&1)))
      %{citation: _} = s -> defs = Enum.concat(s.definitions, Map.get(s.citation, :definitions, []))
        not Enum.any?(s.keywords, & Enum.any?(defs, contains_keyword?.(&1)))
    end)
