  ...
  def render(assigns) do
    ...
    >
      <div class="flex flex-row box-border">
        <%!-- Tier labels column: sits outside the scroll area so it never scrolls away --%>
        <div class="flex flex-col flex-shrink-0 z-40 bg-white">
          <%!-- Corner cell with full extent toggle --%>
          <div class="h-8 border-b border-gray-400 border-r bg-gray-100 flex items-center justify-center">
            <button type="button" data-zoom-fit title="Fit full timeline to view" class="hidden">
              🔍︎ Zoom out
            </button>
          </div>
          <div
            :for={tier <- @tiers}
            class="h-10 border-b border-gray-300 border-r bg-slate-100 px-1 sm:px-2 py-1 text-right whitespace-nowrap text-xs sm:text-sm flex items-center justify-end"
          >
            {tier.name}
          </div>
        </div>

        <%!-- Scrollable timeline + annotations column --%>
        <div class="flex-grow min-w-0 overflow-x-auto" id={"#{@id}-scroll"}>
          <div class="flex flex-col relative">
            ...
            <div
              ...
              data-role="timeline-row"
              style={"width: #{@timeline_width}px;"}
            >
              <div
                :for={tick <- @ticks}
                class="absolute top-0 h-full"
                data-tick-ms={tick.ms}
            ...
            <div
              :for={tier <- @tiers}
              class="h-10 bg-gray-100 relative border-b border-gray-200 overflow-x-clip overflow-y-visible"
              data-role="timeline-row"
              style={"width: #{@timeline_width}px;"}
            >
              <div
                ...
                data-start-ms={anno.start}
                data-end-ms={anno.end}
