defmodule SignbankWeb.SignLive.PhonologicalSearch do
  ...
  def render(assigns) do
    ...
    <form action="/dictionary" method="GET">
      <button class="button">
        <Heroicons.magnifying_glass class="icon--medium" /> Search
      </button>
      ...
      <a phx-click="deselect" phx-value-filter={:location}>
        <Heroicons.x_mark class="has-text-black" style="width: 8em;" />
      </a>
  ...
  def handle_event("deselect", %{"filter" => "handshape"}, socket) do
    socket =
      socket
      |> assign(selected_handshape: nil)
      |> push_event("phon-filter-highlight", %{handshape: nil})

    {:noreply, socket}
  end

  def handle_event("deselect", %{"filter" => "location"}, socket) do
    socket =
      socket
      |> assign(selected_location: nil)
      |> push_event("phon-filter-highlight", %{location: nil})
  ...
  def handshapes(assigns) do
    ...
    <div id="handshapegrid">
      ...
      <a phx-click="deselect" phx-value-filter={:handshape}>
        <Heroicons.x_mark class="has-text-black" />
      </a>
